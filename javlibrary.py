...
from functions_base import JavFile, choose_directory, check_classify_root, check_actors, record_fail,\
    record_video_old, check_extra_folders, check_subt, check_pic, collect_sculpture, replace_xml, replace_xml_win
from functions_baidu import crop_poster_default, tran
from functions_requests import download_pic, steal_arzon_cookies, get_arzon_html, steal_library_header,\
    get_library_html, get_bus_html

#  main开始
print('1、避开21:00-1:00，访问javlibrary和arzon很慢。\n'
      '2、若一直连不上javlibrary，请在ini中更新防屏蔽网址\n'
      '3、如果使用代理，强烈建议开启全局模式，而不是开启ini中的局部代理！\n'
      '   局部代理无法通过javlibrary的5秒检测！\n'
...
try:
    ...
    # 是否 将 系列 作为特征 因为emby不会直接在影片介绍页面上显示片商，也不会读取系列set，所有把它们作为特征
    ...
    for root, dirs, files in os.walk(root_choose):
        ...
        for file_raw in files:
            ...
            if file_temp.endswith(('.SRT', '.VTT', '.ASS', '.SSA', '.SUB', '.SMI',)):
                ...
                subt_num = search(r'([A-Z][A-Z]+)[-_ ]*(\d\d+)', file_temp)  # 匹配字幕车牌
                if str(subt_num) != 'None':
                    jav_pref = subt_num.group(1).upper()
                    # 是素人车牌，不再继续；是常见的无码车牌，不再继续
        ...
        for file_raw in files:
            ...
            if file_temp.endswith(tuple_type) and not file_temp.startswith('.'):
                ...
                for word in list_surplus_words:
                    file_temp = file_temp.replace(word, '')
                video_numg = search(r'([A-Z][A-Z]+)[-_ ]*(\d\d+)', file_temp)  # 匹配视频车牌
        ...
        for jav in list_jav_videos:
            ...
            try:
                ...
                print('    >搜索javlibrary：', url_search_web)
                ...
                if bool_nfo and bool_plot:
                    ...
                    if list_search_results:  # arzon有搜索结果
                        num_results = len(list_search_results)
                        for i in range(num_results):
                            url_on_arzon = 'https://www.arzon.jp' + list_search_results[i]  # 第i+1个链接
                    ...
                    # 没有搜索结果
                        ...
                        if str(adultg) != 'None':
                            num_fail += 1
                            record_fail('    >第' + str(num_fail) + '个失败！arzon成人验证失效，请重新整理该jav的简介：' + path_relative + '\n')
                ...
                if str(coverg) != 'None':
                    ...
                    url_search_bus = url_bus + 'search/' + jav_num + '&type=1&parent=ce'
                    ...
                    if len(list_search_results):  # 搜索结果页面只有一个box
                        num_fail += 1
                        record_fail('    >第' + str(num_fail) + '个警告！系列可能错误，javbus上有多个搜索结果：' + url_search_bus + '，' + path_relative + '\n')
                        url_first_result = list_search_results[0]
                        ...
                        if str(coverg) != 'None':
                            ...
                            num_fail += 1
                            record_fail('    >第' + str(num_fail) + '个失败！查找系列失败，javbus上的链接：' + url_first_result + ' ，请手动添加它的系列。\n')
                    else:
                        num_fail += 1
                        record_fail(
                            '    >第' + str(num_fail) + '个失败！查找系列失败，在javbus上找不到影片信息：' + url_search_bus + '，' + path_relative + '\n')
                #######################################################################
                ...
                if bool_jpg:
                    ...
                    if exists(path_fanart) and check_pic(path_fanart):
                        ...
                        # 已经去过javbus并找到封面
                        if bool_bus_first and url_cover_bus and '图书馆' not in jav.file:
                ...
                if bool_sculpture and jav_epi == 1:
                    if actors[0] == '有码演员':
                        ...
                        for each_actor in actors:
                            ...
                            pic_type = collect_sculpture(path_exist_actor, each_actor)
                            if pic_type == '暂无':
                                num_fail += 1
                                record_fail(
                                    '    >第' + str(num_fail) + '个失败！没有演员头像：' + each_actor + '，' + path_relative + '\n')
                            else:
                                # 已经收录了这个演员头像

        ...
        for jav in list_jav_videos:
            ...
            try:
                ...
                if '仓库' not in jav_file:    # 用户没有指定网址，则去搜索
                    ...
                    list_search_results = findall(r'href="/v/(.+?)" class="box" title=".+?">\s+?<div class=".+?">\s+?<img src=".+?" />\s+?</div>\s+?<div class="uid">(.+?)</div>', html_web, re.DOTALL)  # 匹配处理“标题”

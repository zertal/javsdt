...
from functions_requests import download_pic, steal_arzon_cookies, get_arzon_html, get_bus_html
    ...
    for root, dirs, files in os.walk(root_choose):
        ...
        for file_raw in files:
            ...
            if file_temp.endswith(('.SRT', '.VTT', '.ASS', '.SSA', '.SUB', '.SMI',)):
                ...
                subt_num = search(r'(\d?\d?[A-Z]{1,7}2?8?)[-_ ]*(\d{2,6})', file_temp)  # 匹配字幕车牌
                if str(subt_num) != 'None':
                    jav_pref = subt_num.group(1).upper()
                    # 是素人车牌，不再继续
                    if jav_pref in list_suren_num or jav_pref in ['HEYZO', 'PONDO', 'CARIB', 'TOKYOHOT']:
                        continue    # 【跳出2】
        ...
        for file_raw in files:
            ...
            if file_temp.endswith(tuple_type) and not file_temp.startswith('.'):
                ...
                for word in list_surplus_words:
                    file_temp = file_temp.replace(word, '')
                video_numg = search(r'(\d?\d?[A-Z]{1,7}2?8?)[-_ ]*(\d{2,6})', file_temp)  # 匹配视频车牌
                ...
                if '公交车' not in jav_file:    # 用户没有指定网址，则去搜索
                    ...
                    if list_search_results:  # 搜索页面有结果
                        ...
                        if not list_fit_results:
                            num_fail += 1
                            record_fail('    >第' + str(
                                num_fail) + '个失败！javbus有码找不到该车牌的AV信息：' + url_search_web + '，' + path_relative + '\n')
                            continue           # 【跳出对该jav的整理】
                        # 默认用第一个搜索结果
                    ...
                    # 找不到box
                        ...
                        num_fail += 1
                        record_fail('    >第' + str(
                            num_fail) + '个失败！javbus有码找不到AV信息：' + url_search_web + '，' + path_relative + '\n')
                        continue           # 【跳出对该jav的整理】
                # 用户指定javbus上的网页
                else:
                ...
                if bool_nfo and bool_plot:
                    ...
                    if list_arzon_results:  # arzon有搜索结果
                        ...
                        for i in range(num_results):
                            url_on_arzon = 'https://www.arzon.jp' + list_arzon_results[i]  # 第i+1个链接
                    ...
                    # arzon返回的页面实际是18岁验证
                        ...
                        if str(adultg) != 'None':
                            num_fail += 1
                            record_fail(
                                '    >第' + str(num_fail) + '个失败！arzon成人验证失效，请重新整理该jav的简介：' + path_relative + '\n')

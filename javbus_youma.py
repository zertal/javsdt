    ...
    for root, dirs, files in os.walk(root_choose):
        ...
        for file_raw in files:
            if file_raw.endswith(('.srt', '.vtt', '.ass', '.ssa', '.sub', '.smi',)):
                subt_num = search(r'(\d?\d?[a-zA-Z]{1,7}2?8?)[-_ ]*(\d{2,6})', file_raw)  # 字幕中含有的车牌，字幕车牌 和 视频车牌要配对
        ...
        for file_raw in files:
            if file_raw.endswith(tuple_type) and not file_raw.startswith('.'):
                ...
                video_numg = search(r'(\d?\d?[a-zA-Z]{1,7}2?8?)[-_ ]*(\d{2,6})', file_raw)
        ...
        for jav in list_jav_videos:
            ...
            try:
                    ...
                    if len(list_search_results) == 1:  # 搜索结果页面只有一个box
                        ...
                        for i in list_search_results:
                            ...
                            if jav_suf == url_suf:  # 数字相同
                                url_pref = search(r'([a-zA-Z]+)', url_end).group(1).upper()  # 匹配处理url所带车牌前面的字母“n”
                        ...
                        if not list_fit_results:
                            ...
                            continue
                        url_on_web = list_fit_results[0]   # 默认用第一个
                ...
                if bool_nfo and bool_plot:
                    ...
                    list_arzon[0] = 0
                    list_arzon[1] = '    >打开arzon搜索页面失败，正在重新尝试...'

...
from functions_base import JavFile, choose_directory, check_classify_root, check_actors, record_fail,\
    record_video_old, check_extra_folders, check_subt, check_pic, collect_sculpture, replace_xml_win
from functions_baidu import crop_poster_default, crop_poster_baidu
from functions_requests import download_pic, get_bus_html
    ...
    for root, dirs, files in os.walk(root_choose):
        ...
        for file_raw in files:
            ...
            if file_temp.endswith(('.SRT', '.VTT', '.ASS', '.SSA', '.SUB', '.SMI',)):
                ...
                subt_num = search(r'([A-Z0-9]+)[-_ ]*([A-Z0-9]*[-_ ]*\d*)', file_temp)  # 匹配字幕车牌
        ...
        for file_raw in files:
            ...
            if file_temp.endswith(tuple_type) and not file_temp.startswith('.'):
                ...
                for word in list_surplus_words:
                    file_temp = file_temp.replace(word, '')
                video_numg = search(r'([A-Z0-9]+)[-_ ]*([A-Z0-9]*[-_ ]*\d*)', file_temp)  # 匹配视频车牌
                if str(video_numg) != 'None':
                    ...
                    # jav_num 车牌
                    jav_num = jav_pref + '-' + jav_suf.rstrip()
                    jav_search = jav_num.replace('-', '%20').replace('_', '%20').replace(' ', '%20')
                    # 这个车牌有几集？
                    try:
                ...
                if '公交车' not in jav_file:    # 用户没有指定网址，则去搜索
                    ...
                    if list_search_results:  # 搜索结果页面只有一个box
                        ...
                        if not list_fit_results:
                            num_fail += 1
                            record_fail('    >第' + str(
                                num_fail) + '个失败！javbus无码找不到该车牌的AV信息：' + url_search_web + '，' + path_relative + '\n')
                            continue           # 【退出对该jav的整理】
                        # 默认用第一个搜索结果
                    ...
                    # 找不到box
                        ...
                        num_fail += 1
                        record_fail('    >第' + str(
                            num_fail) + '个失败！javbus无码找不到AV信息：' + url_search_web + '，' + path_relative + '\n')
                        continue           # 【跳出对该jav的整理】
